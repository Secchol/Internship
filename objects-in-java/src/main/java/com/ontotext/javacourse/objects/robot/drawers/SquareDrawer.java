...
public class SquareDrawer implements Drawer {
  ...
  protected void drawRectangle(int x, int y, int length, int width, OntoRobot robot) {
    ...
    Line bottomSide =
        new Line(
            leftSide.getSecondPointX(),
            leftSide.getSecondPointY() + 1,
            leftSide.getSecondPointX(),
            leftSide.getY() + length - 1);
    Line rightSide =
        new Line(
            bottomSide.getSecondPointX() + 1,
            bottomSide.getSecondPointY(),
            x,
            bottomSide.getSecondPointY());
    Line topSide =
        new Line(
            rightSide.getSecondPointX(),
            rightSide.getSecondPointY() - 1,
            leftSide.getX(),
            leftSide.getY() + 1);
